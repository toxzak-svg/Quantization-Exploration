"""Gradient-based mixed-budget Pareto scan for sub1quant.

This driver runs the full Pareto-front experiment end-to-end using **gradient
sensitivity weights** instead of activation-magnitude weights:

  1. Collect per-layer gradient-sensitivity scores from a forward+backward
     pass over WikiText (Fisher-style: W * dL/dW accumulated over chunks).
  2. Run scan_mixed_budget at multiple target_bpw values using those scores.
...
skips if present. Re-runs continue from where they left off. Designed to run
on a single Colab L4 runtime.

Why this exists in parallel to act_weighted_pareto_scan.py:
    Activation-magnitude is a poor proxy for quantization tolerance on
    multimodal transformers. The gradient-sensitivity scoring used here is the
    Fisher-information approximation that GPTQ/AWQ use for mixed-precision
    allocation; it correlates with quantization-induced perplexity
    degradation much better than activation magnitude.

    Once both curves (activation-weighted and grad-weighted) are available,
    we can compare them to decide which scoring the production mixed-budget
    pipeline should use. Whichever curve gives lower PPL at the same target
    avg_bpw wins.

Usage on Colab (L4 GPU, full sweep):

    cd /content/sub1quant
    python3 -u experiments/grad_pareto_scan.py 2>&1 | tee /content/grad_run.log

Poll from Windows (via the colab bridge):

    # from the colab bridge
    exec:  !tail -n 50 /content/grad_run.log
    exec:  !ls /content/sub1quant/eval_results/grad_weighted_scan/
"""
...
DEFAULT_OUT_DIR = PROJECT_ROOT / "eval_results" / "grad_weighted_scan"
...
def ensure_grad_weights(
    ...
    """Run collect_grad_weights.py if grad_weights_gemma4.json is missing."""
    out_dir.mkdir(parents=True, exist_ok=True)
    target = out_dir / "grad_weights_gemma4.json"
    progress = out_dir / "grad_weights_progress.json"

    if target.exists() and json.loads(target.read_text(encoding="utf-8")):
        print(f"[grad] Gradient weights already exist: {target}")
        return target

    if progress.exists():
        progress_data = json.loads(progress.read_text(encoding="utf-8"))
        if progress_data.get("complete"):
            print(f"[grad] Gradient weights complete per progress file: {target}")
            return target

    cmd = [
        sys.executable,
        str(PROJECT_ROOT / "scripts" / "collect_grad_weights.py"),
        "--model-dir", str(model_dir),
        "--wikitext", str(wikitext),
        "--output", str(target),
        "--stats-output", str(out_dir / "grad_stats_gemma4.json"),
    ...
    if rc != 0:
        raise RuntimeError(f"collect_grad_weights.py exited with {rc}")
...
def run_scan(
    ...
    """Run scan_mixed_budget.py at the given target_bpw using the grad weights.

    Note: scan_mixed_budget takes --activation-weights but the field in the
    JSON is just a per-layer scalar weight. We feed the grad-based JSON in
    the same slot — semantics differ (gradient sensitivity vs activation
    magnitude) but the JSON shape is identical.
    """
    ...
    cmd = [
        ...
        "--activation-weights", str(grad_weights),
...
def run_build(
    ...
    output = out_dir / "checkpoints" / f"gemma_mixed_grad_{target_bpw:.2f}.pt"
...
def run_ppl(
    ...
    cmd = [
        ...
        "--label", f"grad_weighted_target_{target_bpw:.2f}",
        "--model-dir", str(model_dir),
        "--wikitext", str(wikitext),
...
def main() -> None:
    ...
    print("=" * 72)
    print("GRADIENT-WEIGHTED MIXED-BUDGET PARETO SCAN")
    print("=" * 72)
    ...
    # Stage 1: collect gradient sensitivity weights
    grad_path = ensure_grad_weights(
        out_dir=out_dir,
        model_dir=model_dir,
        wikitext=wikitext,
    ...
    print(f"[grad] gradient weights: {grad_path} (elapsed {time.time()-t0:.1f}s)")
    print()
    ...
    report = []
    report.append("=" * 72)
    report.append("GRADIENT-WEIGHTED MIXED-BUDGET PARETO RESULTS")
    report.append("=" * 72)
    bf16_ppl = 108.4542
