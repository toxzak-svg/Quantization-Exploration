...
| **Groupwise INT4 (g128)** | 4.13 | 3.88x vs BF16 | full PPL scripted; write `eval_results/groupwise_int4_g128_full_ppl.json` | **PIVOT FOR FP8-LIKE QUALITY** |
...
2. **Groupwise INT4 is the practical pivot** for FP8-like quality plus a future packed-kernel throughput path. The full g128 artifact measured 932.44 MiB, 4.125 BPW, and 0.002849 weighted RMSE in the Colab L4 reconstruction run; the full perplexity gate is now scripted through `scripts/run_publication_ppl.py`. See [FAST_INT4_PIVOT.md](FAST_INT4_PIVOT.md).
